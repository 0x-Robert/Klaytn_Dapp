...
const App = {
  ...
  start: async function () {
   ...
   // console.log('start  function :walletFromSession : ',walletFromSession);
    ...
    if (answer === result){
      if (confirm("대단하네요 ^^ 0.1 KLAY 받기")){
        if(await this.callContractBalance() >= 0.1){
          ...
          alert("죄송합니다. ㅠㅠ 컨트랙의 KLAY가 다 소모되었습니다.");
        }
      }
    }else{
      alert("떙! 초등학생도 하는데 ㅠㅠ~~")
    }
  },
  ...
  changeUI: async function (walletInstance) {
    ...
    // console.log("cavtest",cav.utils.fromPeb(await this.callContractBalance(), "KLAY"));
    // console.log("cav.utils",cav.utils);
    // console.log("this.callContractBalance()",this.callContractBalance());
  ...
  receiveKlay: function () {
    ...
    agContract.methods.transfer(cav.utils.toPeb("0.1", "KLAY")).send({
      ...
      if (receipt.status){
        ...
        $('#transaction').html("");
        $('#transaction').append(`<p><a href='https://baobab.klaytnscope.com/tx/${receipt.transactionHash}' target='_blank'>클레이튼 Scope에서 트랜잭션 확인</a></p>`);
